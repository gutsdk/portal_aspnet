;(function() {
    var lang = { // lang[a.lang].x
        "en": {
    x: "Close", next: "Next month", prev: "Previous month", clear: "Clear",
    ...
    w: "Mo</td><td>Tu</td><td>We</td><td>Th</td><td>Fr</td><td>Sa</td><td>Su"
        ...
        "ru": {
    x: "Закрыть", next: "Следующий месяц", prev: "Предыдущий месяц", clear: "Очистить",
    m: ["Янв", "Фев", "Мрт", "Апр", "Май", "Июн", "Июл", "Авг", "Сен", "Окт", "Ноб", "Дек"],
    ...
    HTM(a.id, '<tbody><tr><th rowspan=4 valign=bottom><a href="" target="_blank">?</a></th>' + ca + '</tr></tbody>');
    ...
    HTM(a.id, '<thead><tr><td class="cal-l" title="' + lang[a.lang].prev + '">‹</td><td colspan=3 class="cal-m"></td><td colspan=2 class="cal-y"></td><td class="cal-r" title="' + lang[a.lang].next + '">›</td></tr><tr><td>' + lang[a.lang].w + '</td></tr></thead>');
    
    if(j > 0) {// ‹ &larr;
        if(m == 0) mm = 11; else mm = m - 1;
        if(j > 1) {
            ca += '<td colspan=' + j + ' class="cal-l" align=left title="' + lang[a.lang].prev + ' ' + lang[a.lang].mo[mm] + '"><b>‹ ' + lang[a.lang].m[mm] + '</b></td>';
        } else ca += '<td class="cal-l" title="' + lang[a.lang].prev + ' ' + lang[a.lang].mo[mm] + '"><b>‹</b></td>';
    }
    ...
    if(DNlast > 0) {// › &rarr;
        ...
        if(DNlast < 6) {
            ca += '<td colspan=' + (7 - DNlast) + ' class="cal-r" align=right title="' + lang[a.lang].next + ' ' + lang[a.lang].mo[mm] + '"><b>' + lang[a.lang].m[mm] + ' ›</b></td>';
        } else ca += '<td class="cal-r" title="' + lang[a.lang].next + ' ' + lang[a.lang].mo[mm] + '"><b>›</b></td>';
    }
